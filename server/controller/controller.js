    ...
    static async register(req, res, next) {
      const t = await sequelize.transaction();
        ...
        try {
          ...
          const data = await User.create({ username, email, password, role, phoneNumber, address }, { transaction: t });
          await t.commit();
          res.status(201).json({ id: data.id, username: data.username, email: data.email, phoneNumber: data.phoneNumber, address: data.address });
        } catch (error) {
          await t.rollback();
          next(error);
        }
      ...
      static async addProduct(req, res, next) {
        ...
        try {
          ...
          const data = await Product.create({ name, description, price, mainImg, categoryId, authorId:req.user.id }, { transaction: t });
          await t.commit();
          res.status(201).json({ id: data.id, name: data.name});
        } catch (error) {
            console.log(error, `<<<<<<<<<<<`);
      ...
      static async detailProduct(req, res, next) {
        try {
          ...
          const temp = await Product.findOne({where: {slug:slug}});
          if (!temp) {
      ...
      static async destroyProduct(req, res, next) {
        ...
        try {
          ...
          res.status(200).json({ message: `Success delete ${nameProduct.name} Product` });
        } catch (error) {
          next(error);
      ...
      static async destroyCategory(req, res, next) {
        ...
        try {
          ...
          res.status(200).json({ message: `Success delete ${nameCategory.name} Category` });
        } catch (error) {
          next(error);
          console.log(error, `<<<<INI ERRORNYA`);
        }
